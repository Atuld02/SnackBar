...
class Recipe extends Component {
	...
	render() {
		const detail = this.state.ingredients ? (
			this.state.ingredients.map((ingredient) => {
				return (
					<li className="collection-item flow-text center" key={Math.random()}>
						{ingredient.originalString}
					</li>
				);
			})
		) : (
			<li className="collection-item flow-text center">
				Ingredients of this recipe is not available
			</li>
		);

		const cuisineChips = this.state.recipe.cuisines
			? this.state.recipe.cuisines.map((cuisine) => {
					return <div className="chip black white-text">{cuisine}</div>;
			  })
			: null;

		const instructionList = this.state.recipe.analyzedInstructions ? (
			this.state.recipe.analyzedInstructions.map((instruction) => {
				return instruction.steps.map((step) => {
					return (
						<li key={Math.random()} className="flow-text">
							{step.step}
						</li>
					);
				});
			...
			<li className="instructions flow-text">
		...
		return (
			<div className="container">
				<div className="section">
					<h4 className="center header" cd recipe>
						{this.state.recipe.title}
					</h4>
					<h6 className="center-align">
						<a href={this.state.recipe.sourceUrl}>
							{this.state.recipe.sourceName}
						</a>
					</h6>
					<div className="row center">
						<FacebookShareButton
							url={
								'https://reacipe.netlify.app/recipe/' +
							...
							<div className="row">
								{this.state.ingredients.length ? (
									<div className="col s6 m6">
										<p className="flow-text center instructions">
											{this.state.ingredients.length}
										</p>
										<p className="flow-text center instructions">
											Ingredients
				...
				<div className="row">
					...
					<div className="center">
						...
						{this.state.recipe.glutenFree ? (
							<div className="chip blue darken-2 white-text">
								Gluten-Free
							</div>
						) : null}
